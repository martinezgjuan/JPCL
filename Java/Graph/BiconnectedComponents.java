 ...
 *       (delete the lines/methods with the mark //*)
...
public class BiconnectedComponents {
	...
	static void dfsBicComp(int parent, int node) {
		...
		for (int next : graph.getAdjacent(node)) {
			...
			if (depth[next] == -1) {
				...
				if (low[next] == depth[next]) // node-next is a bridge 		// *
	...
	public static void main(String[] args) {
		...
		/*
		 * g = new Graph(9); g.addUEdge(0, 1); g.addUEdge(1, 2); g.addUEdge(3,
		 * 4); g.addUEdge(4, 5); g.addUEdge(5, 6); g.addUEdge(6, 7);
		 * g.addUEdge(2, 8); g.addUEdge(8, 1);
		 */
